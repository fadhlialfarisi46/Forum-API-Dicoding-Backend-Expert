...
describe('/threads endpoint', () => {
  ...
  describe('when POST /threads', () => {
    ...
    it('should response 400 when request payload not contain needed property', async () => {
      ...
      expect(responseJson.message).toEqual('Data harus lengkap');
    });
    ...
    it('should response 400 when request payload not meet data type specification', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('Data tidak sesuai');
    });

    it('should response 401 status code when create thread without authentication', async () => {
      // Arrange
      const threadPayload = {
        title: 'Example Title',
        body: 'Example Body',
  ...
  describe('when GET /threads/{threadId}', () => {
    it('should response 200 and persisted detail thread', async () => {
      ...
      await CommentsTableTestHelper.createComment({ id: commentId, owner: userId });
      const replyId = 'reply-123';
      await RepliesTableTestHelper.createReply({ id: replyId, commentId, owner: userId });
